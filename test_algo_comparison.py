...
def test_equivalence(algos, file_name,plot_recharge_time = True, plot_first_example=False, drone_based = True):
    print("* Testing "+algos[0]["algo"].__name__+" and " + algos[1]["algo"].__name__ + " for equivalence.")
    if algo_comparison(algos[0]["algo"], algos[1]["algo"],drone_based=drone_based, plot_first_example=plot_first_example):
        print("* (SUCCESS)")
    else:
        if(plot_recharge_time):
            print("* Plotting difference between algorithms over all inputs")
            if not drone_based:
                plot_total_recharge_time_SMILP_DATA(algos, file_name=file_name[:-4] + "(SBLP).eps")
                plot_total_recharge_time(algos, file_name=file_name[:-4] + "(DBLP).eps", legend_outside=False)
            else:
                plot_total_recharge_time(algos, file_name=file_name, legend_outside=False)
        print("* (FAIL)")
    print("---------------------------------------------------------------")
